...
function autoGenerateDraws() {
  ...
  Object.keys(groups).forEach(key => {
    ...
    if (type === 'Grupos + Eliminatoria') {
      // Preferencia: numGroups/qualifiers do range do perfil. Fallback: calculo BTP ideal.
      // Grupos+Elim exige minimo 2 grupos (senao vira round-robin); garantimos min 2.
      const range = getDrawRangeForCount(tournament, drawPlayers.length);
      newDraw.numGroups = Math.max(2, range?.numGroups || calcIdealGroupCount(drawPlayers.length));
      newDraw.groupQualifiers = range?.groupQualifiers || 2;
    }
...
function setRanges(ranges){const c=document.getElementById('gp-ranges-container');let h='';ranges.forEach((r,i)=>{
  ...
  // Auto-expandir avancado se o perfil ja tem valores customizados (diferentes do default 2/2)
  const hasCustomGroups = isGrupos && ((r.numGroups && r.numGroups !== 2) || (r.groupQualifiers && r.groupQualifiers !== 2));
  h+=`<div class="form-row" style="margin-bottom:12px;align-items:stretch;background:#fff;border:1px solid #E2E8F0;border-radius:8px;padding:12px 14px;gap:12px;flex-wrap:wrap">
    ...
    <button type="button" class="range-advanced-toggle" onclick="toggleRangeAdvanced(this)" data-expanded="${hasCustomGroups?'1':'0'}" style="${isGrupos?'':'display:none;'}background:transparent;border:1px dashed #CBD5E1;color:#64748B;padding:4px 10px;border-radius:6px;cursor:pointer;font-size:11px;font-weight:600" title="Configuracao avancada — sobrescreve o calculo automatico">
      ${hasCustomGroups?'&minus;':'+'} Avancado
    </button>
    <div class="range-advanced-panel" style="display:${hasCustomGroups?'flex':'none'};align-items:center;gap:8px;width:100%;padding:10px;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:6px" data-grupos-config>
      <div style="font-size:11px;color:#64748B">Override automatico:</div>
      <div style="font-size:11px;color:#64748B;text-align:center"><div>Grupos</div><input type="number" class="form-control range-groups" value="${r.numGroups||''}" placeholder="auto" min="1" max="8" style="width:60px;text-align:center;margin-top:2px" title="Deixe vazio para usar calculo automatico BTP"></div>
      <div style="font-size:11px;color:#64748B;text-align:center"><div>Classif.</div><input type="number" class="form-control range-qualifiers" value="${r.groupQualifiers||''}" placeholder="auto" min="1" max="4" style="width:60px;text-align:center;margin-top:2px" title="Quantos classificam por grupo (padrao: 2)"></div>
      <span style="font-size:10px;color:#94A3B8;margin-left:auto">Vazio = calcula automaticamente pela quantidade de atletas</span>
    </div>
...
// Expande/colapsa o painel "Avancado" de uma linha de range
function toggleRangeAdvanced(btn) {
  const row = btn.closest('.form-row');
  if (!row) return;
  const panel = row.querySelector('.range-advanced-panel');
  if (!panel) return;
  const expanded = btn.dataset.expanded === '1';
  if (expanded) {
    panel.style.display = 'none';
    btn.dataset.expanded = '0';
    btn.innerHTML = '+ Avancado';
  } else {
    panel.style.display = 'flex';
    btn.dataset.expanded = '1';
    btn.innerHTML = '&minus; Avancado';
  }
}
function onRangeTypeChange(){
  document.querySelectorAll('#gp-ranges-container .form-row').forEach(row=>{
    ...
    const toggleBtn = row.querySelector('.range-advanced-toggle');
    const panel = row.querySelector('.range-advanced-panel');
    if (toggleBtn) toggleBtn.style.display = isGrupos ? '' : 'none';
    // Ao mudar pra non-Grupos, colapsa o painel automaticamente
    if (!isGrupos) {
      if (panel) panel.style.display = 'none';
      if (toggleBtn) { toggleBtn.dataset.expanded = '0'; toggleBtn.innerHTML = '+ Avancado'; }
    }
    row.querySelectorAll('[data-qualif-config]').forEach(el=>{el.style.display=isGrupos?'':'none';});
  });
}
...
function collectRanges(){
  ...
  document.querySelectorAll('#gp-ranges-container .form-row').forEach(row => {
    ...
    if (entry.type === 'Grupos + Eliminatoria') {
      // Campos vazios significam "usar calculo automatico BTP" (nao sobrescrever).
      // Apenas salvar se o admin explicitamente preencheu um numero.
      const gv = row.querySelector('.range-groups')?.value?.trim();
      const qv = row.querySelector('.range-qualifiers')?.value?.trim();
      if (gv) entry.numGroups = Math.max(1, Math.min(8, parseInt(gv)));
      if (qv) entry.groupQualifiers = Math.max(1, Math.min(4, parseInt(qv)));
    }
    r.push(entry);
  });
  return r;
...
// Retorna o range COMPLETO do perfil que se aplica a um count de atletas (ou null).
// Usado para herdar numGroups e groupQualifiers pre-configurados pelo admin.
function getDrawRangeForCount(t, count) {
  if (!t?.gameProfileId) return null;
  const p = gameProfiles.find(x => x.id === t.gameProfileId);
  if (!p || p.mode === 'fixed') return null;
  for (const r of (p.ranges || [])) if (count >= r.min && count <= r.max) return r;
  return p.ranges?.length ? p.ranges[p.ranges.length - 1] : null;
}

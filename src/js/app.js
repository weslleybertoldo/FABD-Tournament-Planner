...
async function loadData(autoLoad=false) {
  try {
    ...
    if (tournament) {
      ...
      try {
        prepareRankingsForSync();await window.api.supabaseUpsertTournament(tournament.id, tournament.name, tournament);
...
async function savePlayer() {
  try {
    ...
    try{prepareRankingsForSync();await window.api.supabaseUpsertTournament(tournament.id,tournament.name,tournament);}catch(e){console.warn('Sync Supabase:',e);}
...
function renderDrawDetail(idx) {
  ...
  if (has && d.type==='Eliminatoria') h += renderBracket(d);
    ...
    if (d.groupsData && areGroupsFinished(d) && !d.groupsData.eliminationGenerated) {
      ...
      // Usar syncMatchesWithDraws em vez de rebuildMatchList pra nao destruir matches existentes
      syncMatchesWithDraws();
    }
...
async function toggleAwarded(idx){
  ...
  d.awarded=!d.awarded;
  await window.api.saveTournament(tournament);
  try{prepareRankingsForSync();await window.api.supabaseUpsertTournament(tournament.id,tournament.name,tournament);}catch(e){console.warn('Sync premiacao:',e);}
  renderDraws();
...
function propagateGroupsToElimination(d) {
  ...
  // Cross-seed: 1st of group A vs 2nd of group B, etc.
  const numGroups = d.groupsData.groups.length;
  const orderedPlayers = [];
  // For 2 groups: A1 vs B2, B1 vs A2
  // For N groups: snake seeding
  for (let seedPos = 0; seedPos < qualifiers; seedPos++) {
    const fromGroups = qualified.filter(q => q.seed === seedPos);
    if (seedPos % 2 === 0) {
      fromGroups.forEach(q => orderedPlayers.push(q.name));
    } else {
      fromGroups.reverse().forEach(q => orderedPlayers.push(q.name));
    }
  }

  // Generate elimination bracket
  const elimMatches = generateEliminationBracket(orderedPlayers, []);
  elimMatches.forEach(m => { m.phase = 'elimination'; });
...
async function assignCourt(idx, value) {
  ...
  try{
    ...
    else if(!value){await window.api.supabaseRemoveFromCourt(tournament.id,m);prepareRankingsForSync();await window.api.supabaseUpsertTournament(tournament.id,tournament.name,tournament);}
...
async function saveScore() {
  try{
    ...
    propagateResultToDraws(m);
    await window.api.saveTournament(tournament);try{prepareRankingsForSync();await window.api.supabaseUpsertTournament(tournament.id,tournament.name,tournament);}catch(e){console.warn('Sync Supabase:',e);}closeModal('modal-score');renderMatches();showToast('Placar salvo!');
  }catch(e){console.error(e);showToast('Erro: '+e.message,'error');}
}
...
async function saveTournamentConfig(){
  try{
    ...
    syncEntriesFromPlayers();
    await window.api.saveTournament(tournament);try{prepareRankingsForSync();await window.api.supabaseUpsertTournament(tournament.id,tournament.name,tournament);}catch(e){console.warn('Sync Supabase:',e);}closeModal('modal-tournament-config');renderTournamentPage();showToast('Configuracao salva!');
  }catch(e){showToast('Erro: '+e.message,'error');}
}
